...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		foreach ($this->_styles as $style) {
			$template['styles'][] = '<link href="'. base_url($style) .'" rel="stylesheet">';
		}
		$template['styles'] = implode("\n\t\t", $template['styles']);

		$template['scripts'] = array();
		foreach ($this->_scripts as $script) {
			$template['scripts'][] = '<script src="'.base_url($script).'"></script>';
		}
		$template['scripts'] = implode("\n\t\t", $template['scripts']);

        $template['inlineScripts'] = implode("\n\t\t", $this->_inlineScripts);
	...
	public function append_scripts($scripts)
	{
		$this->_scripts = array_merge($this->_scripts, $scripts);
	}

	public function append_styles($scripts)
	{
		$this->_styles = array_merge($this->_styles, $scripts);
	}

    public function append_inline_script($script)
    {
        $this->_inlineScripts[] = '<script>' . $script . '</script>';
    }
    ...
    public function remove_partial($name) {
        $name = !is_array($name) ? array($name) : $name;
        foreach ($name as $nm) {
            unset($this->_partials[$nm]);
        }
    }
